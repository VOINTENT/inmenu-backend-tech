...
class CuisineTypeService(BaseService):
    ...
    @staticmethod
    async def get_cuisine_types(pagination_size: int, pagination_after: int, lang_id: int):
        cuisine_types, error_cuisine_types = CuisineTypeDao().get_list_cuisine_type(pagination_size, pagination_after,
                                                                                    lang_id)
