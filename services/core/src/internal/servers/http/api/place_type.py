...
from src.internal.biz.services.place_type import PlaceTypeService
...
async def get_place_types(request: Request, pagination_size: int, pagination_after: int, lang_id: int):
    place_types, err = PlaceTypeService.get_all_place_types(pagination_size, pagination_after, lang_id)
    if err:
        return err.get_response_with_error()

    return get_response_get_place_types(place_types)
